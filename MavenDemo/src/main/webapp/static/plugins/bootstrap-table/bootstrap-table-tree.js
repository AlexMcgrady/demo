(function ($) {
    ...
    var getParent = function (node, source, options) {
        var data = [];
        var items = $.grep(source, function (item, index) {
            return node[options.parentId] == item[options.treeId];
        });
        $.each(items, function (index, item) {
            data.splice(0, 0, item);
            var child = getParent(item, source, option);
    ...
    var getChild = function (node, source, options) {
        var data = [];
        var items = $.grep(source, function (item, index) {
            return item[options.parentId]== node[options.treeId];
        });

        $.each(items, function (index, item) {
        	item.level = node.level + 1;
            data.push(item);
            var child = getChild(item, source, options);
            $.each(child, function (i, n) {
                data.push(n);
            });
        });
    ...
    BootstrapTable.prototype.initData = function () {
        ...
        if (that.options.treeView && this.data.length > 0) {
            ...
            var roots = $.grep(this.data, function (row, index) {
            	var rowParentId = row[that.options.parentId];
            	if (rowParentId != null) {
            		var dataArr = that.data;
            		var firstLevel = true;
            		for(var k=0; k < dataArr.length; k++) {
            			if (rowParentId == dataArr[k][that.options.treeId]) {
            				firstLevel = false;
            				break;
            			}
            		}
            ...
            $.each(roots, function (index, item) {
            	item.level = 1;
                rows.push(item);
                var child = getChild(item, that.data, that.options);
                $.each(child, function (i, n) {
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        ...
                        var hiddenChild = $.grep(data, function (d, i) {
                            return d[that.options.parentId]== item[that.options.treeId];
                        });
                        var iconClass = 'childIndent';
                        if (child.length > 0) {
                        	iconClass = that.options.expandIcon;
                        } else if(hiddenChild.length > 0) {
                        		iconClass = that.options.collapseIcon;
                        }
        ...
        this.$body.find('> tr[data-index] > td > .tree-icon').off('click').on('click', function (e) {
            ...
            var $this = $(this),
                ...
                row = data[index];
            var icon = $(this);
            var child = getChild(data[index], data, that.options);
